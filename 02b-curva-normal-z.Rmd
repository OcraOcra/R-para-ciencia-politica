...
```{r norm-density, echo=FALSE, fig.cap="Densidad de una distribucion normal simulada con media 50 y desviacion estandar 12,5", out.width='90%', fig.align='center'}
m <- mean(datos2); s <- sd(datos2)
ggplot(data.frame(x = datos2), aes(x = x)) +
  geom_density(fill = "slategray2") +
  geom_vline(xintercept = m, size = 1) +
  geom_vline(xintercept = m + s, linetype = 2, size = 1, color = "green4") +
  geom_vline(xintercept = m - s, linetype = 2, size = 1, color = "green4") +
  geom_vline(xintercept = m + 2 * s, linetype = 4, size = 1, color = "goldenrod3") +
  geom_vline(xintercept = m - 2 * s, linetype = 4, size = 1, color = "goldenrod3") +
  annotate("text", x = 78, y = 0.028, label = "1 DE", color = "green4", size = 5) +
  annotate("text", x = 92, y = 0.017, label = "2 DE", color = "goldenrod3", size = 5) +
  scale_x_continuous(expand = c(0, 0)) +
  labs(title = "Datos de una distribucion normal (n = 1.000.000)",
       subtitle = "media = 50 y desviacion estandar = 12,5",
       x = "Valores en el eje X", y = "Densidad") +
  theme_minimal()
```
