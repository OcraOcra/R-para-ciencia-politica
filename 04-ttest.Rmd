...
```{r figure1, echo=FALSE, fig.cap="La distribucion normal estandar"}
normal_prob_area_plot <- function(lb, ub, mean = 0, sd = 1,
  ...
  area <- data.frame(x = areax, ymin = 0,
                     ymax = dnorm(areax, mean = mean, sd = sd))
  (ggplot()
   + geom_line(data.frame(x = x, y = dnorm(x, mean = mean, sd = sd)),
               mapping = aes(x = x, y = y))
   + xlab("95% de los valores estan entre +/- 2 desviaciones estandar") + ylab("")
   + geom_ribbon(data = area, fill = "grey80",
...
stats <- test %>%
  summarise(
    ...
    Mediana = median(edad),
    Varianza = var(edad),
    `Desviacion estandar` = sd(edad)
  )
stats2 <- t(stats)
kable(format(stats2, digits = 4, drop0trailing = TRUE), align = "cc",
      col.names = c("Estadisticas descriptivas"))
```
...
```{r fig-margen-error, echo=FALSE, fig.cap="Margen de error segun el tamano de la muestra"}
n <- seq(100, 2500, 1)
me <- 100 * 1.96 * (0.50 / sqrt(n))
plot(me ~ n, ylab = "Margen de error (%)", xlab = "Tamano de la muestra")
```
...
```{r figure, echo=FALSE, fig.cap="Distribucion de la nota a los partidos politicos"}
ggplot(ciep, aes(x = as.numeric(nota_pp))) +
  geom_histogram(binwidth = 1, fill = "#3182bd", color = "black") +
  labs(x = "Nota a los partidos (0 a 10)", y = "Frecuencia")
```

**Tabla 5.2 Estadisticas descriptivas. Nota a los partidos politicos**

```{r tbl-nota-pp, echo=FALSE}
test <- ciep %>% drop_na(nota_pp)
stats <- test %>%
  summarise(
    N = n(),
    Media = mean(as.numeric(nota_pp)),
    Mediana = median(as.numeric(nota_pp)),
    Varianza = var(as.numeric(nota_pp)),
    `Desviacion estandar` = sd(as.numeric(nota_pp))
  )
stats2 <- t(stats)
kable(format(stats2, digits = 4, drop0trailing = TRUE), align = "cc",
      col.names = c("Estadisticas descriptivas"))
...
```{r gender_pp, echo=FALSE}
test <- ciep %>% drop_na(nota_pp, sexo_f) %>%
  mutate(nota_pp = as.numeric(nota_pp))
res <- t.test(nota_pp ~ sexo_f, data = test)
cat("Media para Mujeres =", round(res$estimate[1], 2), "\n",
    "Media para Hombres =", round(res$estimate[2], 2), "\n",
    "t =", round(unname(res$statistic), 2), "\n",
    "p =", sprintf("%.3f", res$p.value), "\n")
```
...
```{r vote_gob, echo=FALSE}
test <- ciep %>% drop_na(nota_gob, votopac_f) %>%
  mutate(nota_gob = as.numeric(nota_gob))
res <- t.test(nota_gob ~ votopac_f, data = test)
cat("Media para 'No voto PAC' =", round(res$estimate[1], 2), "\n",
    "Media para 'Voto PAC' =", round(res$estimate[2], 2), "\n",
    "t =", round(unname(res$statistic), 2), "\n",
    "p =", sprintf("%.3f", res$p.value), "\n")
```
