...
```{r figure2, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="Diagrama de dispersion: nota al gobierno y apoyo al sistema"}
figure2 <- ciep %>% filter(!is.na(b6n), !is.na(nota_gob))
ggplot(figure2, aes(x = b6n, y = as.numeric(nota_gob))) +
  geom_point() +
  ylab("Nota al gobierno") +
...
```{r figure3, warning=FALSE, echo=FALSE, message=FALSE, fig.cap="Grafico de barras: media de la nota al gobierno por nivel de apoyo al sistema"}
ciep %>%
  filter(!is.na(b6n), !is.na(nota_gob)) %>%
  group_by(b6n) %>%
  summarise(media = mean(as.numeric(nota_gob))) %>%
  ggplot(aes(x = factor(b6n), y = media)) +
  geom_bar(stat = "identity", fill = "#3182bd", color = "black") +
  ylab("Nota media al gobierno") +
  xlab("Apoyo al sistema (1 a 7)")
```

**Figura \@ref(fig:figure4) Grafico de lineas. Nota al gobierno y apoyo al sistema**

```{r figure4, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="Grafico de lineas: media de la nota al gobierno por nivel de apoyo al sistema"}
figure4 <- ciep %>%
  filter(!is.na(b6n), !is.na(nota_gob)) %>%
  group_by(b6n) %>%
  summarise(media = mean(as.numeric(nota_gob)))

ggplot(figure4, aes(x = b6n, y = media)) +
  geom_line() +
  ylab("Nota media al gobierno") +
  xlab("Apoyo al sistema (1 a 7)")
```
...
```{r figure5, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="Edad y nota media a los partidos politicos"}
figure5 <- ciep %>%
  filter(!is.na(edad), !is.na(nota_pp)) %>%
  group_by(edad) %>%
  summarise(media = mean(as.numeric(nota_pp)))

ggplot(figure5, aes(x = edad, y = media, group = 1)) +
  geom_line() +
  ylab("Nota media a los partidos") +
  xlab("Edad (anos)")
```
...
```{r example, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Ejemplos de correlacion positiva, negativa y nula"}
...
```{r table2, echo=FALSE}
df <- ciep %>% select(nota_gob, nota_pp) %>% mutate(across(everything(), as.numeric)) %>% drop_na()
...
df <- ciep %>%
  transmute(
    `Apoyo al sistema` = b6n,
    `Nota al gobierno` = as.numeric(nota_gob),
    `Nota a partidos` = as.numeric(nota_pp),
    `Nota a la UCR` = as.numeric(nota_ucr),
