...
```{r fig-percepcion, echo=FALSE, fig.cap="Percepcion de la situacion economica del pais, 2020"}
ciep %>%
  filter(!is.na(sit_f)) %>%
  ggplot(aes(x = sit_f)) +
  geom_bar(fill = "#3182bd", color = "black") +
  ylab("Frecuencia") +
  xlab("Situacion economica del pais")
```
...
```{r table56, echo=FALSE}
test1 <- chisq.test(table(ciep$sexo_f, ciep$educ_f))
cat("n =", round(sum(test1$expected), 0),
    " Chi-cuadrado =", round(unname(test1$statistic), 2),
    ", p =", sprintf("%.3f", test1$p.value),
    ", gl =", test1$parameter, "\n")
cat("\n")

test2 <- chisq.test(0.10 * table(ciep$sexo_f, ciep$educ_f))
cat("n =", round(sum(test2$expected), 0),
    " Chi-cuadrado =", round(unname(test2$statistic), 2),
    ", p =", sprintf("%.3f", test2$p.value),
    ", gl =", test2$parameter, "\n")
```
